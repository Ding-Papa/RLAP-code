...
class DQN:
    ...
    def select_action(self, cond, choices):
        ...
        for choice in choices:
            output = self.tokenizer(choice + ' ' + cond + self.eos_token)
    ...
    def getBatch(self):
        ...
        new_cond, new_choices = zip(*next_state)
        input_ids = []
        attention_mask = []

        for (cond, choices), action in zip(state, action):
            output = self.tokenizer(choices[action] + ' ' + cond + self.eos_token)
            input_ids.append(torch.IntTensor(output['input_ids'][:1024]))
            attention_mask.append(torch.IntTensor(output['attention_mask'][:1024]))
