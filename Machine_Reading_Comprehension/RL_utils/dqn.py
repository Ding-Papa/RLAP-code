...
class DQN:
    ...
    def select_action(self, ques, now_ans, context, choices):
        # Exploration
        ...
        for choice in choices:
            output = self.tokenizer(context + ' ' + choice, ques)
            input_ids.append(torch.IntTensor(output['input_ids'][:1024]))
            attention_mask.append(torch.IntTensor(output['attention_mask'][:1024]))
    ...
    def select_next_action(self, ques, now_ans, context, choices):
        if choices == []:
    ...
    def getBatch(self):
        ...
        ques, new_ans, new_context, new_choices = zip(*next_state)
        input_ids = []
        attention_mask = []
        for (qu, now_ans, context, choices),action in zip(state, action):
            output = self.tokenizer(context + ' ' + choices[action], qu)
            input_ids.append(torch.IntTensor(output['input_ids'][:1024]))
            attention_mask.append(torch.IntTensor(output['attention_mask'][:1024]))
        ...
        return input_ids, attention_mask, \
                reward, \
                ques, new_ans, new_context, new_choices, \
    ...
    def update(self):
        ...
        next_q = torch.tensor([self.select_next_action(q,a,c,ch) for (q,a,c,ch) in zip(ques, new_ans, new_context, new_choices)]).cuda()
